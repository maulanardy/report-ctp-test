...
const sheet1 = [
  {
    ...
    data: [
      ...
      [
        {
          value: `Daftar 15 besar transaksi Surat Berharga Negara yang dilaporkan melalui BEI ${moment().format(
            'DD MMMM Y'
          )}`,
      ...
      [
        {
          value: `List of Top 15 Government Securities Transaction Report ${moment().format('LL')}`,
...
Object.keys(DataBySecurities).forEach(d2 => {
  if (Array.isArray(DataBySecurities[d2])) {
    const trx = DataBySecurities[d2].filter(
      d => moment(d.trade_date).diff(moment(), 'day') === 0 && parseInt(d.securities_type_id, 10) === 1
    );
  ...
  render() {
    return (
      <>
        ...
        <ExcelFile filename={`Top 15 Government PLTE (${currency})`}>
